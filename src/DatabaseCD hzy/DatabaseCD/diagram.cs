...
namespace DatabaseCD
{
    public partial class diagram : Form
    {
        ...
        public diagram(int cid,string colname,string title)
        {
            ...
            else { mysql = "select hwl=case hwl	when '1' then  '1小时以下'when '2' then  '1-2小时'when '3' then  '2-3小时'when '4' then  '3-4小时'when'5' then '4小时以上' else '无数据' end	,count(hwl) from Assessment where cID=" + cid.ToString() + "group by hwl"; }
